...
def get_tokenized(texts, tokenizer, max_seq_len):
    ...
    for text in texts:
        ...
        # Để đảo bảo khi thêm các token [CLS, SEP], chiều dài không vượt quá max_seq_len
        if len(tokenized_res) > max_seq_len - 3:
            tokenized_res = tokenized_res[:max_seq_len - 3] # cắt chuỗi
        ids = tokenizer.convert_tokens_to_ids(tokenized_res)  # chuyển token thành các id tương ứng
        len1 = len(ids) // 2

        x = [101] + ids[:len1] + [102] + ids[len1:] + [102] # chia nhỏ chuỗi thành 2 phần
        mask = [1] * len(x)
        ...
        assert len(x) == max_seq_len, "Giá trị độ dài của x không phải là max_seq_len"
        assert len(mask) == max_seq_len, "Giá trị độ dài của mask không phải là max_seq_len"
